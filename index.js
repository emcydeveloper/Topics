...
//const movie = client.db("dbName").collection("movie").findone({id:"102"})
...
app.get('/movies', async function (request, response) {
    console.log(request);
    // const movie = await clientConnection.db("we-batch27").collection("movies");
...
app.get('/movies/:id', async function (request, response) {
    ...
    //Connecting to DB and table
    const movie = await clientConnection.db("we-batch27").collection("movies").findOne({id:id})
    // movies.filter((mv)=>mv.id==id)
    response.send(movie)
})
